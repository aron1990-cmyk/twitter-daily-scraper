...
class SystemMonitor:
    ...
    def __init__(self, 
        ...
        # P0+优化：增加主动内存清理配置
        self.memory_cleanup_threshold = 60.0  # 内存使用超过60%时主动清理（从70%降低）
        self.memory_critical_threshold = 75.0  # 内存使用超过75%时激进清理
        self.last_memory_cleanup = datetime.now()
        self.last_critical_cleanup = datetime.now()
            ...
            try:
                ...
                if metrics:
                    ...
                    if metrics.memory_percent > self.memory_critical_threshold:
                        time_since_last_critical = (datetime.now() - self.last_critical_cleanup).total_seconds()
                        # 激进清理：间隔2分钟执行一次
                        if time_since_last_critical > 120:
                            self._perform_critical_memory_cleanup(metrics.memory_percent)
                            self.last_critical_cleanup = datetime.now()
                    elif metrics.memory_percent > self.memory_cleanup_threshold:
                        time_since_last_cleanup = (datetime.now() - self.last_memory_cleanup).total_seconds()
                        # 常规清理：间隔3分钟执行一次（从5分钟缩短）
                        if time_since_last_cleanup > 180:
                            self._perform_memory_cleanup(metrics.memory_percent)
                            self.last_memory_cleanup = datetime.now()
    ...
    def _perform_memory_cleanup(self, current_memory_percent: float):
        """P0+优化：执行常规内存清理"""
        try:
            ...
            try:
                ...
                if task_manager and hasattr(task_manager, 'clear_completed_tasks'):
                    # 使用异步方式调用清理
                    import asyncio
                    if hasattr(asyncio, 'create_task'):
                        loop = asyncio.get_event_loop()
                        if loop and loop.is_running():
                            asyncio.create_task(task_manager.clear_completed_tasks(older_than_hours=0.3))
                        else:
                            # 如果没有运行的事件循环，使用同步方式
                            asyncio.run(task_manager.clear_completed_tasks(older_than_hours=0.3))
            except Exception as e:
                self.logger.warning(f"清理任务管理器缓存失败: {e}")
            
            # 3. 清理浏览器管理器（如果存在）
            try:
                from utils.browser_manager import BrowserManager
                # 触发浏览器清理（这里只是记录，实际清理由浏览器管理器自己处理）
                self.logger.info(f"[P0+优化] 触发浏览器资源清理检查")
            except Exception as e:
                self.logger.warning(f"触发浏览器清理失败: {e}")
            ...
            self.logger.info(
                f"[P0+优化] 常规内存清理完成: {memory_before:.1f}% → {memory_after:.1f}% "
                f"(节省 {memory_saved:.1f}%)"
            )
            
        except Exception as e:
            self.logger.error(f"[P0+优化] 常规内存清理失败: {e}")
    
    def _perform_critical_memory_cleanup(self, current_memory_percent: float):
        """P0+优化：执行激进内存清理"""
        try:
            memory_before = current_memory_percent
            
            self.logger.warning(f"[P0+优化] 内存使用率达到临界值 {memory_before:.1f}%，开始激进清理")
            ...
            try:
                ...
                if task_manager and hasattr(task_manager, 'clear_completed_tasks'):
                    ...
                    if hasattr(asyncio, 'create_task'):
                        ...
                        if loop and loop.is_running():
                            # 激进清理：清理所有已完成任务
                            asyncio.create_task(task_manager.clear_completed_tasks(older_than_hours=0.1))
                        else:
                            asyncio.run(task_manager.clear_completed_tasks(older_than_hours=0.1))
            except Exception as e:
                self.logger.warning(f"激进清理任务管理器缓存失败: {e}")
            
            # 3. 清理监控历史数据
            if len(self.metrics_history) > 100:
                # 保留最近100条记录，删除其余
                old_count = len(self.metrics_history)
                self.metrics_history = deque(list(self.metrics_history)[-100:], maxlen=self.metrics_history.maxlen)
                self.logger.info(f"[P0+优化] 清理监控历史数据: {old_count} → {len(self.metrics_history)}")
            
            # 4. 清理进程指标缓存
            if len(self.process_metrics) > 50:
                # 只保留最近的50个进程指标
                sorted_processes = sorted(self.process_metrics.items(), key=lambda x: x[1].memory_mb, reverse=True)
                self.process_metrics = dict(sorted_processes[:50])
                self.logger.info(f"[P0+优化] 清理进程指标缓存，保留前50个")
            ...
            self.logger.warning(
                f"[P0+优化] 激进内存清理完成: {memory_before:.1f}% → {memory_after:.1f}% "
                f"(节省 {memory_saved:.1f}%)"
            )
            
            # 如果清理后内存仍然很高，记录警告
            if memory_after > 80.0:
                self.logger.error(f"[P0+优化] 警告：激进清理后内存仍然很高 ({memory_after:.1f}%)，建议检查内存泄漏")
            
        except Exception as e:
            self.logger.error(f"[P0+优化] 激进内存清理失败: {e}")
    
    def cleanup_old_data(self, days: int = 3) -> int:  # P0优化：从7天改为3天
        """
        P0优化：清理旧的监控数据文件，缩短保留时间
        ...
        try:
            ...
            if cleaned_count > 0:
                self.logger.info(
                    f"[P0优化] 清理了 {cleaned_count} 个旧的监控数据文件，释放 {total_size_mb:.2f}MB 磁盘空间"
                )
            
        except Exception as e:
            self.logger.error(f"[P0优化] 清理旧数据失败: {e}")
