...
class RefactoredTaskManager:
    ...
    def _start_task_with_user(self, task_id: int, user_id: str, use_background_process: bool) -> bool:
        ...
        try:
            # 获取任务信息（在锁外进行）
            from web_app_optimized import ScrapingTask, db
            from flask import has_app_context
    ...
    def _start_background_process(self, task_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            # 启动进程
            cmd = ['python3', 'utils/background_task_runner.py', config_file.name]
    ...
    def _start_thread_task(self, task_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            # ScrapingTaskExecutor可能不存在于web_app_optimized中，我们需要检查
            import asyncio
    ...
    def stop_task(self, task_id: int) -> Tuple[bool, str]:
        ...
        if slot:
            try:
                ...
                # 更新数据库中的任务状态为已停止
                try:
                    from web_app_optimized import ScrapingTask, db, app
                    with app.app_context():
                        task = ScrapingTask.query.get(task_id)
                        if task:
                            task.status = 'stopped'
        ...
        try:
            ...
            while not self.task_request_queue.empty():
                try:
                    priority, task_request = self.task_request_queue.get_nowait()
                    if task_request.task_id == task_id:
                        found_in_queue = True
                        print(f"[RefactoredTaskManager] 从队列中移除任务 {task_id}")
                        # 更新数据库状态
                        try:
                            from web_app_optimized import ScrapingTask, db, app
                            with app.app_context():
                                ...
                                if task:
                                    ...
                                    task.completed_at = datetime.utcnow()
                                    db.session.commit()
                                    print(f"[RefactoredTaskManager] 队列任务 {task_id} 状态已更新为停止")
                        except Exception as db_e:
                            print(f"[RefactoredTaskManager] 更新队列任务状态失败: {str(db_e)}")
                    else:
                        # 保留其他任务
                        queue_items.append((priority, task_request))
                except queue.Empty:
                    break
            
            # 将其他任务放回队列
            for item in queue_items:
                self.task_request_queue.put(item)
            
            if found_in_queue:
                return True, "队列中的任务已停止"
            else:
                return False, "任务未在运行中或队列中"
                
        except Exception as e:
            print(f"[RefactoredTaskManager] 从队列中停止任务 {task_id} 失败: {str(e)}")
            return False, f"停止队列任务失败: {str(e)}"
