...
async def main():
    ...
    主函数
    """
    logger.info(f"🎯 开始抓取剩余{len(REMAINING_BLOGGERS)}个博主的推文...")
    ...
    try:
        ...
        # 逐个抓取博主
        for i, username in enumerate(REMAINING_BLOGGERS, 1):
            logger.info(f"\n📊 进度: {i}/{len(REMAINING_BLOGGERS)} - 当前博主: @{username}")
            ...
            # 休息一下再继续
            if i < len(REMAINING_BLOGGERS):
                logger.info("⏱️ 休息5秒后继续下一个博主...")
                await asyncio.sleep(5)
        
        # 输出最终统计
        logger.info(f"\n🎉 剩余博主抓取完成!")
        logger.info(f"📈 成功抓取: {successful_count}/{len(REMAINING_BLOGGERS)} 个博主")
        logger.info(f"📝 新增推文数: {total_tweets} 条")
        logger.info(f"📊 平均每个博主: {total_tweets/successful_count:.1f} 条推文" if successful_count > 0 else "📊 平均每个博主: 0 条推文")
        
    except Exception as e:
        logger.error(f"❌ 剩余博主抓取失败: {e}")
