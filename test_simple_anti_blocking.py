...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('test_simple_anti_blocking.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

async def test_simple_anti_blocking():
    """
    简化的反封控机制测试
    """
    parser = None
    ...
    try:
        logger.info("🚀 开始测试简化的反封控机制")
        
        # 1. 创建Twitter解析器（使用默认调试端口）
        logger.info("创建Twitter解析器...")
...
async def main():
    ...
    """
    logger.info("=" * 60)
    logger.info("Twitter反封控机制简化测试")
    logger.info(f"测试时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"目标用户: 0xluffy_eth")
    logger.info("=" * 60)
    
    success = await test_simple_anti_blocking()
